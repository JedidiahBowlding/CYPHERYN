...
export default function DetectionsPage() {
  ...
  return <SectionPage title="Detection engineering" eyebrow="Phase 10" description="Manage portable detections and correlate authorized network telemetry with investigation assets.">
    ...
    <div className="detection-actions">
      <form onSubmit={importSigma}><span className="eyebrow">Sigma</span><h2>Import a detection rule</h2><p>Validated YAML with title, logsource, detection selections, and condition.</p><input type="file" accept=".yml,.yaml,text/yaml" onChange={(event) => setSigmaFile(event.target.files?.[0] ?? null)} /><button disabled={!sigmaFile} aria-label="Import Sigma rule" title="Import Sigma rule">Import</button></form>
      <form onSubmit={importLog}><span className="eyebrow">Network telemetry</span><h2>Import JSON-lines logs</h2><p>Suricata EVE alerts or Zeek JSON logs, limited to 10 MiB and 5,000 events.</p><select value={logSource} onChange={(event) => setLogSource(event.target.value)}><option value="suricata">Suricata EVE</option><option value="zeek">Zeek JSON</option></select><input type="file" accept=".json,.jsonl,.ndjson" onChange={(event) => setLogFile(event.target.files?.[0] ?? null)} /><button disabled={!logFile || !authorized} aria-label="Import network telemetry" title="Import network telemetry">Import</button></form>
    </div>
    <div className="detection-export"><button onClick={() => download("sigma")} aria-label="Export Sigma rules" title="Export Sigma rules">Sigma</button><button onClick={() => download("suricata")} aria-label="Export Suricata rules from STIX" title="Export Suricata rules from STIX">Suricata</button></div>
